...
class ModelTester:
    def __init__(self, logger, restore_snap=None, on_cpu=False):
        ...
        if restore_snap is not None:
            ...
            DP.log_string("Model restored from " + restore_snap, self.log_out)
    ...
    def inference(self, model, dataset, use_votes=False, num_votes=100):
        ...
        while last_min < num_votes:
            ...
            try:
                ...
                step_id += 1
                DP.log_string('Epoch {:3d}, step {:3d}. min possibility = {:.1f}'.format(epoch_id, step_id, np.min(
                    dataset.min_possibility[dataset.cloud_split])), self.log_out)

            except tf.errors.OutOfRangeError:

                # Save predicted cloud
                new_min = np.min(dataset.min_possibility[dataset.cloud_split])
                DP.log_string('Epoch {:3d}, end. Min possibility = {:.1f}'.format(epoch_id, new_min), self.log_out)
                ...
                if last_min + 4 < new_min or not use_votes:
                    ...
                    # Project predictions
                    DP.log_string('Reproject Vote #{:d}'.format(int(np.floor(new_min))), self.log_out)
                    ...
                    t2 = time.time()
                    DP.log_string('[ModelTester.inference] Done in {:.1f} s\n'.format(t2 - t1), self.log_out)
                    self.sess.close()
                    return preds_list
