...
class DataSetSemantic3D:
    def __init__(self, need_sample=True, leaf_size=50, label2Names=LABEL_MAP):
    ...
    def set_input_clouds(self, data_list):
        for pc in data_list:
            ...
            pc_colors = sub_colors / 255.0
            self.input_colors[self.cloud_split] += [pc_colors]
    ...
    def get_batch_gen(self):
        ...
        def spatially_regular_gen():
            ...
            for i in range(num_per_epoch):  # num_per_epoch
                ...
                query_idx = self.input_trees[self.cloud_split][cloud_idx].query(pick_point, k=cfg.num_points)[1][0]
                ...
                self.min_possibility[self.cloud_split][cloud_idx] = float(np.min(self.possibility[self.cloud_split][cloud_idx]))
