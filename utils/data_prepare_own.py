...
for pc_path in glob.glob(join(dataset_path, extend)):
    print(pc_path)
    ...
    if exists(label_path):
        ...
        labelDistribution.append(Counter(sub_labels))
        ...
        full_ply_path = join(original_pc_folder, file_name + '.ply')
        write_ply(full_ply_path, (pc[:, :3].astype(np.float32), pc[:, 3:6].astype(np.uint8)),
                  ['x', 'y', 'z', 'red', 'green', 'blue'])

        # save sub_cloud and KDTree file
        sub_xyz, sub_colors = DP.grid_sub_sampling(pc[:, :3].astype(np.float32), pc[:, 3:6].astype(np.uint8),
                                                   grid_size=grid_size)
        sub_colors = sub_colors / 255.0
        sub_ply_file = join(sub_pc_folder, file_name + '.ply')
...
distribution = {}
for cal in labelDistribution:
    for key in cal.keys():
        if key in distribution.keys():
            distribution[key] += cal[key]
        else:
            distribution[key] = cal[key]
